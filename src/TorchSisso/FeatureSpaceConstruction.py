...
class feature_space_construction:
  ...
  '''
    ...
    def combinations(self, operators_set, i):
        operators_set = tuple(sorted(operators_set))
        if operators_set in self.combination_cache:
            if i in self.combination_cache[operators_set]:
                return self.combination_cache[operators_set][i]
            else:
                self.combination_cache[operators_set][i] = None
        else:
            self.combination_cache[operators_set] = {}
            self.combination_cache[operators_set][i] = None
        ...
        feature_values11 = torch.empty(
            self.df.shape[0], 0).to(self.device)

        for op in operators_set:
            feature_names_11 = []
            self.feature_values11 = feature_values11.clone()
        ...
        if len(self.feature_names_binary) == 0:
            self.combination_cache[operators_set][i] = self.feature_values_binary, self.feature_names_binary
