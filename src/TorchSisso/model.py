...
class SissoModel:
    ...
    def fit(self):
        ...
        if self.dimensionality == None:
            ...
            if self.operators == None:
                sys.exit(
                    'Please provide the operators set for the non dimensional Regression!!')
            ...
            if self.multi_task != None:
                ...
                for i in range(len(self.multi_task_target)):
                    ...
                    print('Performing symbolic regression of',
                          i+1, 'Target variables....')

                    list1 = []

                    list1.extend([self.multi_task_target[i]] +
                                 self.multi_task_features[i])

                    df1 = self.df.iloc[:, list1]

                    x, y, names = fc.feature_space_construction(self.operators, df1, self.no_of_operators, self.device,
                                                                self.initial_screening, self.custom_unary_functions, self.custom_binary_functions).feature_space()

                    from .Regressor import Regressor

                    rmse, equation, r2, final_eq = Regressor(
                        x, y, names, self.dimension, self.sis_features, self.device, self.use_gpu).regressor_fit()
                ...
                x, y, names = fc.feature_space_construction(self.operators, self.df, self.no_of_operators, self.device,
                                                            self.initial_screening, self.custom_unary_functions, self.custom_binary_functions).feature_space()
            ...
            if self.multi_task != None:
                ...
                for i in range(len(self.multi_task_target)):
                    ...
                    x, y, names, dim = dfc.feature_space_construction(
                        self.df, self.operators, self.relational_units, self.initial_screening, self.no_of_operators, self.device, self.dimensionality).feature_expansion()

                    # print(names)
                    from .Regressor_dimension import Regressor

                    rmse, equation, r2 = Regressor(x, y, names, dim, self.dimension, self.sis_features,
                                                   self.device, self.output_dim, self.regressor_screening, self.use_gpu).regressor_fit()
